...
pub(crate) fn get_compat_os_data_dir(app_handle: &AppHandle) -> Result<PathBuf, String> {
  ...
  let public_dir_desktop = base_dir.join("public");
  let public_dir_android = base_dir.join("files/public");
  match family() {
    "linux" => Ok(public_dir_desktop),
    "windows" => Ok(public_dir_desktop),
    "macos" => Ok(public_dir_desktop),
    "android" => Ok(public_dir_android),
    _ => Err(format!("Unsupported operating system: {}", family())),
  }
}

pub(crate) fn ensure_os_data_dir(app_handle: &AppHandle) -> Result<PathBuf, String> {
  let public_dir = get_compat_os_data_dir(app_handle)?;
  if !public_dir.exists() {
    std::fs::create_dir_all(&public_dir).map_err(|e| e.to_string())?;
  }
  Ok(public_dir)
}
