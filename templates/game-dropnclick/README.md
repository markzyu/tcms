...
yarn test        # jest + jsdom (mocks window.tcms.cdnBridge)
...
All three files contain calls to Node.js modules. But `src/content` files rely on helper functions to do so. If you need custom Node.js logic, please add those to `esbuild.config.mts` or add a new, configurable build behavior in `@tcms/mini-app-common`.

## Testing (TDD)

Component tests use Jest + jsdom + React Testing Library. They do **not** run esbuild or hit the network.

Use `@tcms/mini-app-react-test-utils` to install a mocked `window.tcms.cdnBridge`. Bind your template content type once per test file; content is supplied via **`initialContentJson`** (v0.1 does not call `fetchContentJson`):
