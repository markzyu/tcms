...
- These apps should support SSR and generate new HTML based on the content provided by the users. But they don't have to write the HTML themselves or build them. Instead, they simply need to serve the static content on a port. And the local CDN will cache that and serve the page properly
